...
void seralize_add_user_input(ShortPublicKey spk, MasterSecretKey msk, std::vector<std::vector<std::string>> members,std::string partitions_info, std::string user_add,std::string& in_buffer);
void deralize_add_user_input(std::string in_buffer,ShortPublicKey& spk,MasterSecretKey& msk,std::string &group_key,std::string &user_add,std::vector<SpibbePartition>& partitions);
void serialize_add_user_output(unsigned char* sealed_group_key, std::vector<SpibbePartition> partitions, std::string& out_buffer);
